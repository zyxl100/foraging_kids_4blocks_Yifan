...
function get_sub_crp_latents(sub_num, generative_model_num, alpha_level)
    return deserialize("fit_data/gen_"*get_model_name(generative_model_num)*"_fit_"*get_model_name(alpha_level)*"/crp_latents/sub"*string(sub_num)*".ser")
end 
...
function main()
    ...
    sub_data = get_sub_crp_latents(sub_num, gen_model_num,alpha_level)
    ...
    file_name = string("fit_data/gen_"*get_model_name(gen_model_num)*"_fit_"*get_model_name(alpha_level)*"/neg_llh/sub"*string(sub_num)*".csv")
