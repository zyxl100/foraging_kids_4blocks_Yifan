...
function evaluation_uncertainty_adapt(reward::Float64, planet_decays::Vector{Any}, global_rr::Float64, stay_time::Float64, N::Int64, bundle_of_particles::Vector{Particle}, w::Vector{Float64}, sim_samples::Int64, alpha::Float64, g_base::Float64, g_coef::Float64)
    ...
    return v_stay, v_leave, maximum(clusters)
...
function crp(params,num_particles=200)
    ...
    for b in 1:exp.n_blocks
        ...
        while (time_in_block < exp.max_time_in_block) & (planet_num_in_block < exp.max_planets_in_block)
            ...
            v_stay, v_leave, max_clus = evaluation_uncertainty_adapt(init_reward, planet_decays, total_reward/total_time, get_time_constant("stay", timing), total_planet_n, particles, weights, n_samples, alpha, gamma_base, gamma_coef)
            ...
            while action == 1
                ...
                v_stay, v_leave, max_clus = evaluation_uncertainty_adapt(reward, planet_decays, total_reward/total_time, get_time_constant("stay", timing), total_planet_n, particles, weights, n_samples, alpha, gamma_base, gamma_coef)
                action = select_action(v_stay, v_leave, beta, eps)
    ...
    return simulated_data, max_clus, total_reward
end
...
#     beta = rand(Uniform(0.0, 5.0))

#     return [alpha, gamma_base, gamma_coef, beta]
# end 
