...
function initialize_planet(data, planet_num)
    ...
    rt = curr_trial.rt[1]
    return curr_planet, n_stay_decisions, planet_decays, init_reward, rt
end 
...
function get_reward_uncertainty(alpha, particles, particle_weights, n_samples, n_planet, last_reward, curr_planet_decays)
    ...
    samples, clusters = sample_v_stay(particles, particle_weights, prob_k, n_samples, curr_planet_decays) # sample from decay rate distributions 
    
    clusters = round.(Int, clusters)
    K = maximum(clusters)
    dist = Multinomial(n_samples,tally(clusters,K)/n_samples)
    uncertainty = entropy(dist)

    next_reward = mean(samples)*last_reward # predict the reward that will be recieved on the next trial 
    return next_reward, uncertainty
end
...
function crp_only(data, exp, timing, alpha, num_particles=200)
    ...
    all_predictions = NamedTuple{(:choice_num, :pred_reward, :reward_uncert, :global_rr, :planet_stay_num, :max_planet_stay_num, :rt)}[]
    for p in 0:max_planets 
        curr_planet, n_stay_decisions, planet_decays, init_reward, rt  = initialize_planet(data, p)
        ...
        push!(all_predictions,(;choice_num,pred_reward,reward_uncert,global_rr,planet_stay_num,max_planet_stay_num,rt))
...
function monte_carlo_sample_latents(alpha_level, sub_num, data, n_runs=1000)
    ...
    serialize("crp_latents_alpha_"*string(alpha_level)*"/sub"*string(sub_num)*".ser", crp_predictions)
end

function get_sub_data(sub_num)
    all_data = DataFrame(CSV.File("data/all_data_20240417.csv"))
    return all_data[in(sub_num).(all_data.sub_num),:]
end 
...
function main()
    ...
    monte_carlo_sample_latents(alpha_level,sub_num,sub_data)
end
