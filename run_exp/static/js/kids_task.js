...
var begin_practice = {
  type: 'html-button-response',
  stimulus: practice_game_txt,
  choices: ['re-read instructions', 'play the practice game'],
  on_finish: function(data) {
    data.trial_type = "begin_practice";

    if (data.button_pressed == '0') {
      jsPsych.addNodeToEndOfTimeline({
        timeline: [welcome, move_explain, welcome_dec, lock_choice],
      }, jsPsych.resumeExperiment);
    } else {
      jsPsych.addNodeToEndOfTimeline({
        timeline: [alien_welcome_prac, landing_prac, dig_prac, harvest_prac, decision_prac],
